...
// check liveness, and close -- so a test can drive the lock mechanics against a
...
type pinnedConn interface {
	...
	// ping reports whether the session is still alive (connection death releases the
	// lock, the basis of failover).
...
func (c *pgxSessionConn) ping(ctx context.Context) error { return c.conn.Ping(ctx) }
