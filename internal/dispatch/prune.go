...
// This file is the pure count-based retention decision: which run rows the
// dispatcher's opportunistic post-pass prune targets. It is pure -- no I/O, no meta
// access -- so retention is a function of the run ids alone: keep the newest `retain`
// runs per pipeline and prune the rest, sparing any run still held by an outstanding
// dead_letters entry. The write that archives and deletes a pruned run is
// store.PruneRun; this file owns only the decision the dispatcher feeds it.
...
// the pipeline it belongs to. Retention is count-based and clockless, so the run id
// -- meta's monotonic identity, never a clock -- orders a pipeline's runs
// newest-first, and the pipeline groups them. Nothing else about a run bears on
// whether it is prunable: no timestamp, and -- deliberately -- no consumer watermark,
// since the gate reads only an upstream's latest run and never pins an older one
// against retention (consumption and retention are unlinked).
