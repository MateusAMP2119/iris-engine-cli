...
// process and asserts only the runner mechanics (the real binary is built,
// invoked, and its stdout/stderr/exit code captured) and the exit-code categories
// -- a bare invocation exits with the success code, and an unknown argument exits
// 2, the usage-error category, with the usage error on stderr. It deliberately
// asserts no stdout SHAPE: exit codes are pinned, not the words the binary
// prints, so this test stands unchanged when E01 replaces the placeholder with
// the cobra tree (bare iris printing root help instead of a version, still exit
// 0).
//
...
func TestConformanceRealBinaryJSON(t *testing.T) {
	...
	t.Run("bare invocation exits with the success code", func(t *testing.T) {
		// Exit-code category 0. Exit code only, never stdout shape: E01's
	...
	t.Run("unknown argument exits 2 with a usage error on stderr", func(t *testing.T) {
		// Exit-code category 2 (usage error); detail lives in the message.
