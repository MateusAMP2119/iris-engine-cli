...
// applies. Capture is unconditional: every write to a declared table is stamped in the
// journal regardless of the pipeline's artifact or data mode. What the mode governs is
// the payload TIER of that stamp, never whether the stamp is written:
//
...
//     revert budget -- a wipe restores those pre-images. Full pre-images are kept only
//     where undo spends them.
...
// write from an un-built source pipeline: a loose source never writes permanent data.
...
// mirrored here so the tier model and the journal agree.
type WriteOp string
