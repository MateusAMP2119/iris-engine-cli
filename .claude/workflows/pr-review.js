...
const COMMON = `You are reviewing a PR in the Iris engine repo at ${REPO}. Scope: ${scope}. The diff under review is \`git diff ${base}...${head}\` — run it (read-only) to see exactly what changed; review THE DIFF, using the rest of the tree as context. Read CLAUDE.md first (conventions + TDD doctrine). Spec: docs/Iris Specification Inventory.md (source of truth); epic contract tables: docs/Iris Epics.md; task briefs: docs/Tasks/.
