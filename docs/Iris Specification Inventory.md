...
# quickstart - a root onboarding verb: the installer's handoff, a guided tour of the first session (interactive only on a real terminal)
...
**Q - Quickstart surface?** A: `iris quickstart` is the third root verb, the installer's handoff: an interactive guided tour that explains, confirms, then really runs the first-session path — `engine install`, `engine start -d`, `engine info`, materialize the embedded sample, `declare apply`, `pipeline run`, `iris data provenance` on a row it wrote — ending with the engine left running and a printed cheat-sheet. A tutor, never a second engine surface: every step executes the real command implementation through the tour's own binary (never a PATH lookup), same code path, same exit codes. It operates on cwd as the workspace like every command; when cwd is not a workspace it proposes creating `./iris-quickstart-demo` and works there. Adaptive because the commands are idempotent: a reachable daemon on the workspace socket announces install/start as already done and skips them; sample files are written only when absent, and a present-but-different file is kept and warned about, never clobbered. Declining or Ctrl-C aborts clean: exit `0`, a resume hint, nothing half-broken. A failing step surfaces that command's own error and exit category. `--yes` runs every step unattended; `--host` is refused (usage error, exit `2` — the tour provisions a local engine), and an ambient `IRIS_HOST` or config-file host is ignored with a notice — the tour only ever targets the local workspace engine it provisions; `--socket` stays accepted.

**Q - Quickstart interactivity gating?** A: Interactivity requires stdin AND stdout to be interactive terminals and `--json` off; anything else gets the plain guide — the complete numbered copy-paste rendering of the same steps, byte-stable plain text, zero ANSI, executing nothing, exit `0`. Color follows the ceremony rule exactly (interactive stdout, `NO_COLOR` unset, `--json` off): `NO_COLOR` strips paint, never interactivity. Under `--json` the guide rides the one data envelope: the ordered step list (id, explanation, argv), executing nothing.

**Q - Quickstart sample pipeline?** A: Embedded in the binary, golden-pinned, materialized on demand into the workspace: `pipelines/hello_iris/` (declaration + POSIX-sh script, `run: [sh, main.sh]`, no lane so a manual run is immediate and synchronous) and `schemas/demo/colors/table.yaml` (pk `name` text). The script upserts the seven rainbow colors over the injected `IRIS_DB_URL` (psql from PATH, else the managed `.iris/pg/bin/psql` found by upward walk), fixed pks, `ON CONFLICT DO UPDATE`: re-running layers a second stamp on the same rows — the provenance lesson. The embedded declaration and table file must always parse through the real loaders: an invalid sample is a test failure, never a runtime surprise.

**Q - Installer handoff?** A: install.sh ends by offering the tour: with a controlling terminal (probed by opening `/dev/tty`, the uninstaller's rule) and `IRIS_FORCE` unset it prompts `Take the 3-minute guided tour? (Y/n)` reading from `/dev/tty`; accept (the default) execs `"$dest/iris" quickstart </dev/tty` — the absolute path so the tour runs even when `~/.local/bin` is not yet on PATH, the re-tied stdin satisfying the TTY gate. Decline, no terminal, or `IRIS_FORCE=1` prints the plain next-steps lines and exits `0`. Prose over a shell script: exempt, proven by the quickstart contracts it delegates to.
