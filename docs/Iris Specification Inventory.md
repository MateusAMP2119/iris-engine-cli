...
iris engine connect [host]     # point this workspace at a remote engine: verify host + PAT, record them in .iris/iris.toml (0600)
...
# update - a root lifecycle verb: self-replace the iris binary itself (daemonless self-update; distinct from engine install/uninstall, which manage engine state)
iris update                    # self-replace the installed binary with the latest GitHub release (daemonless)

# uninstall - a root lifecycle verb: remove the iris binary itself (daemonless self-removal; distinct from engine uninstall, which tears down engine state)
iris uninstall                 # delete the running iris executable (gated: --yes / TTY y-N; refuses while a daemon is reachable unless --force)

# quickstart - a root onboarding verb: the installer's continuation, a guided tour of the first session (interactive only on a real terminal)
iris quickstart                # explain, confirm, and really run the first-session path; piped or --json: the numbered copy-paste guide / step-list envelope, executing nothing
```
...
**Q - Update surface?** A: `iris update` is a root lifecycle verb (a counterpart of `iris uninstall`): it self-replaces the installed binary with the latest GitHub release, mirroring the curl installer (`install.sh`) without a package manager, and is daemonless (it touches no daemon). It resolves the latest tag by following the redirect of `.../releases/latest` to `.../releases/tag/<tag>` and reading the tag from the final URL path (no GitHub API JSON, so no rate-limit exposure), then compares it to the running `<build>`. A `dev` build refuses (it was not installed from a release): the command fails with guidance to use the curl installer or `go install`, exit `4`. When the latest tag equals the running version it reports already-up-to-date and exits `0`, downloading nothing. Otherwise it downloads `iris_<GOOS>_<GOARCH>.tar.gz` and `checksums.txt` from `.../releases/download/<tag>/`, verifies the archive's SHA-256 against the matching `checksums.txt` line (a mismatch aborts, touching no binary, exit `4`), extracts the `iris` member, and replaces the running executable atomically (write a sibling temp file in the executable's own directory, `chmod 0755`, rename over it; the executable is resolved through its symlinks first). A permission failure on replace prints guidance to re-run with `sudo` or reinstall with the curl installer and exits `4`. Success exits `0`. Under `--json` the outcome (status, running version, latest tag, replaced path) rides the one data envelope.

**Q - Uninstall surface?** A: `iris uninstall` is a root lifecycle verb: it removes the installed `iris` binary itself, the mirror of the curl uninstaller (`uninstall.sh`) and the counterpart of `iris update`. It is distinct from `iris engine uninstall`, which tears down engine state (`meta`, the journal, the object store, the socket, the service unit): the root verb touches only the executable file. It is daemonless (it dials no daemon) and never listens (no remote surface). It first probes for a reachable daemon by the same socket/host resolution every command uses; while one is reachable it refuses with guidance to run `iris engine stop && iris engine uninstall` first and exits `4`, unless `--force` overrides the probe. It is gated like a dev-loop destructive op: `--yes` (or `--force`) confirms non-interactively; without either, a TTY is prompted `Uninstall <version> from <path>? (y/N)` and anything but `y`/`yes` aborts cleanly (exit `0`, `Aborted. Nothing removed.`, touching no file); without `--yes` and without a terminal it fails with the standard consent-required error, exit `4`. It resolves the running executable through its symlinks (`os.Executable` then `EvalSymlinks`) and removes that file; a permission failure prints guidance to re-run with `sudo` or use the curl uninstaller and exits `4`. Success prints `Uninstalled <path>.` then `Goodbye from iris.` and exits `0`; under `--json` the outcome (status, version, removed path) rides the one data envelope.

**Q - Lifecycle terminal ceremony?** A: The two root lifecycle verbs (`iris update`, `iris uninstall`) paint a colored terminal journey mirroring the curl installer and uninstaller (`install.sh`/`uninstall.sh`), but only when their stdout is an interactive terminal, `NO_COLOR` is unset, and `--json` is off; otherwise output stays byte-identical plain text, so no ANSI escape ever reaches a pipe, a file, or a machine consumer. `iris update` renders staged progress as it works — resolving the latest release, downloading the platform asset (with its size), verifying the SHA-256, swapping the binary — then a green-check summary (`✓ updated iris <from> -> <to>`); an up-to-date run renders a single green-check line. The staged progress is driven by an optional per-stage progress hook on the updater (stages, in order: resolve, download, verify, replace), nil by default so `internal/update` stays a stdlib-only, silent leaf unless the CLI supplies the renderer. `iris uninstall` draws a cyan confirmation box before its `y/N` prompt, a green abort line on decline, and on success a rainbow farewell (`Goodbye`, one bright color per letter) after `Uninstalled <path>.`. The plain-mode strings are unchanged (`updated iris <from> -> <to>`, `iris is already up to date (version <v>)`, `Uninstalled <path>.`, `Goodbye from iris.`, `Aborted. Nothing removed.`), so piped and `--json` behavior is exactly as before and every pinned output contract holds.

**Q - Quickstart act ceremony?** A: The tour's chapters are named, never numbered: THE CLI (install.sh's act), THE ENGINE, THE PIPELINE. A chapter opens with a light rule-and-title mark — `── THE ENGINE ──────…` — the uninstall.sh box family at lighter weight, one bright palette color per act along the shared rainbow (yellow for THE CLI, cyan for THE ENGINE, magenta for THE PIPELINE); the sign-off is the per-rune rainbow. Step progress inside an act rides the `iris update` staged grammar (`→ verb … detail`, `✓ summary`). The artwork renders only under the ceremony rule (interactive stdout, `NO_COLOR` unset, `--json` off); the piped plain guide restructures the same steps under plain-text act headings, golden-pinned, and the installer's non-TTY output keeps today's plain strings byte-identical.

**Q - Quickstart surface?** A: `iris quickstart` is the third root verb, the installer's continuation: a chaptered guided tour that explains, then really runs the first session in two acts — THE ENGINE (the workspace question, `engine install`, `engine start -d`, the `engine info` readout) and THE PIPELINE (materialize the embedded sample, `declare apply`, `pipeline run`, `iris data provenance` on a row it wrote) — ending with the engine left running and a printed cheat-sheet. Consent is per act, never per step: one affirmative opens an act, and its steps run straight through as a staged journey; the first failing step stops the tour with that command's own error and exit category. The ENGINE act opens with the workspace question: `Engine workspace [~/iris]:` — one line read with a visible default, `~` expanded to the operator's home, the directory created (mkdir -p) and entered, so every subsequent step operates on cwd exactly like any command. The default is `~/iris`, never the invoking directory — under `curl | sh` the invoking directory is arbitrary — except when the invoking directory is already a workspace (`.iris/` or `pipelines/` present), which the prompt proposes back as the default. Unattended (`--yes`) never prompts and uses the invoking directory as the workspace unchanged. `--from-installer` is the installer's continuation entry: it opens directly on the ENGINE chapter — the installer's banner was the welcome, its Y/n the act's consent — and is otherwise the same tour; combined with `--json` it stays the inert step-list envelope (the installer's version probe). A tutor, never a second engine surface: every step executes the real command implementation through the tour's own binary (never a PATH lookup), same code path, same exit codes. Adaptive because the commands are idempotent: a reachable daemon on the workspace socket announces install/start as already done and skips them; either way THE ENGINE act closes only on the readout — after its steps the tour polls the daemon (bounded, about ten seconds) until it reports leadership (role `leader` — a contending or standby daemon is not ready, since the tour's workspace engine must win its own election before a mutation can land), and a daemon that never does is a clear fault (exit `4`): THE PIPELINE act never opens on an unready engine; sample files are written only when absent, and a present-but-different file is kept and warned about, never clobbered. Declining or Ctrl-C aborts clean: exit `0`, a resume hint, nothing half-broken. `--yes` runs every step unattended; `--host` is refused (usage error, exit `2` — the tour provisions a local engine), and an ambient `IRIS_HOST` or config-file host is ignored with a notice — the tour only ever targets the local workspace engine it provisions; `--socket` stays accepted.

**Q - Quickstart interactivity gating?** A: Interactivity requires stdin AND stdout to be interactive terminals and `--json` off; anything else gets the plain guide — the complete numbered copy-paste rendering of the same steps, byte-stable plain text, zero ANSI, executing nothing, exit `0`. Color follows the ceremony rule exactly (interactive stdout, `NO_COLOR` unset, `--json` off): `NO_COLOR` strips paint, never interactivity. Under `--json` the guide rides the one data envelope: the ordered step list (id, explanation, argv, act) plus the catalog object (ids, pitches, showcases, the default and the selected entry), executing nothing.

**Q - Quickstart pipeline catalog?** A: The tour's pipeline act opens at a catalog: curated starter pipelines embedded in the binary (go:embed, no network), golden-pinned, one folder per entry — an `entry.yaml` (id matching its folder, display name, one-line pitch, description, run note, and the showcase table and pk the finale queries) beside a `workspace/` subtree mirroring exactly what materializes into the workspace. An ordered `catalog.yaml` index lists exactly the entry folders, ids unique, entry 1 `hello_iris` — the default pick; the index shape is servable from a remote catalog later. Starters obey the starter constraints: POSIX sh plus psql over the injected `IRIS_DB_URL` (psql from PATH, else the managed `.iris/pg/bin/psql` found by upward walk), fixed pks with `ON CONFLICT DO UPDATE` so re-running layers a second provenance stamp on the same rows, no lane so a manual run is immediate and synchronous, and no other hard dependency. Every entry must parse through the real declare loaders: an invalid entry is a test failure, never a runtime surprise. Interactively the act is a shop: the numbered name-and-pitch list, `Pick a pipeline (1-N, Enter=1):`, the empty answer taking entry 1; a pick prints the entry's description and finale preview, and the ordinary confirms then drive materialize, apply, run, and the finale. `--pipeline <id>` picks explicitly in every rendering (an unknown id is a usage error, exit `2`, naming the available ids); `--yes` without `--pipeline` takes the default. The plain guide and the `--json` envelope carry the catalog and execute nothing.

**Q - Remote connect surface?** A: `iris engine connect [host]` points a workspace at an engine that already runs elsewhere — the recorded counterpart of the ad-hoc `--host`/`--token` pair. It resolves the host (argument > `--host`/`IRIS_HOST`/`iris.toml` > interactive prompt) and the PAT (`--token`/`IRIS_TOKEN` > interactive hidden prompt — the PAT never echoes), verifies the pair against the engine's PAT-gated `GET /healthz` before anything is recorded, then upserts `host` and `token` into the invoking directory's `.iris/iris.toml` (written `0600`, an existing file tightened to `0600` — this write may be the one that adds a token), the configuration layer every subsequent command in that workspace resolves through. The upsert owns only the keys it sets: every other line — comments, `pg_dsn`, any hand-edited setting — survives verbatim, and a duplicate of an owned key is dropped (the parser is last-one-wins, so a kept duplicate would override the rewrite). The failure modes stay distinct and record nothing: an unreachable host is no-daemon (exit `3`, like every dial), a rejected PAT or any other non-2xx answer is an operation failure (exit `4`) naming what the engine said, and a missing host or PAT without a terminal is a usage error (exit `2`). Scheme-aware like every TCP dial: `https://host:port` verifies over TLS, bare `host:port` stays plain. Under `--json` the outcome (host, role, config path — never the token) rides the one data envelope.

**Q - Quickstart engine-home fork?** A: The interactive tour opens on a fork before any consent or act: `Where does your engine live?` — Local (provision on this machine: the heads-up and the acts, unchanged) or Remote (connect to an existing engine). The ceremony surface asks with the arrow-key select; a terminal that refuses raw mode gets the numbered line dialogue, whose empty answer takes Local — the fork never makes the plain first run longer than one Enter. `--yes` and every harnessed seam take Local unattended: the fork is production dialogue only. The Remote branch is `iris engine connect` walked as dialogue: the heads-up note (what to obtain from the engine's operator, what is and is not touched locally), the host question, the hidden PAT question, the verification probe — a failed probe explains itself and re-asks in place rather than ending the tour — then the workspace question (the same question THE ENGINE act opens with, anchoring the same thing: where subsequent commands resolve from) and the recorded connection, closing on its own wrap-up (the first commands worth running against the remote, how to disconnect, and that the local sandbox tour is one `iris quickstart` away). It provisions nothing, starts nothing, and never opens the acts; `q`, EOF, and Ctrl-C abort clean at any question (exit `0`), and nothing is recorded on an aborted or failed branch.

**Q - Quickstart ceremony widgets?** A: The clack-style widget kit the interactive tour renders with — the rail intro/outro and bars, boxed notes (terminal-width aware, wrapped on spaces with long words hard-broken), the arrow-key select (Up/Down or j/k, 1-9 jump, Enter picks, q/Esc/Ctrl-C decline), the Yes/No radio confirm, the braille spinner, and the block-art act banner — is hand-rolled on the shared painter, raw terminal mode via x/term, no TUI framework. Production dialogue only: every widget requires raw mode and reports ok=false (rendering nothing) when the terminal refuses it, so the caller falls back to the cooked line dialogue; the injectable tour seams bypass the kit entirely, so harnessed tests never meet raw mode; and a disabled painter renders the plain byte-stable surface — the banner paints nothing, the spinner degrades to plain `·`/`✓` lines, no ANSI ever reaches a non-terminal consumer. An interrupted widget restores the terminal's cooked state before any abort line prints.

**Q - Installer handoff?** A: `curl … | sh` is the guide itself: install.sh performs THE CLI act — its banner, download, checksum, and installed lines, kept verbatim — then hands the ceremony to the binary it installed. The handoff is version-gated by probing that binary: `"$dest/iris" quickstart --from-installer --json` exiting `0` means the full chaptered ceremony (exec `"$dest/iris" quickstart --from-installer </dev/tty` — the absolute path so the tour runs even when `~/.local/bin` is not yet on PATH, the re-tied stdin satisfying the TTY gate); plain `quickstart --json` exiting `0` means the flagless handoff of an older tour-bearing binary; anything older still gets no handoff and no quickstart next-step line. One question, with a controlling terminal (probed by opening `/dev/tty`, the uninstaller's rule) and `IRIS_FORCE` unset: `Set up the engine now? (Y/n)` reading from `/dev/tty`, default yes — the single Y/n serving as the ENGINE act's consent, which is why `--from-installer` opens without a welcome or act gate of its own. Decline, no terminal, or `IRIS_FORCE=1` prints the plain next-steps lines (the quickstart line only when a probe passed) and exits `0`. Prose over a shell script: exempt, proven by the quickstart contracts it delegates to.
